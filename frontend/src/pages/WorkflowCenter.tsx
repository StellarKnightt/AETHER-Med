...
import { 
  ...
  Share2, Loader2, Network, ChevronRight, X, AlertTriangle, PlayCircle,
  Stethoscope, ShieldCheck, HeartPulse, Clock, FileBarChart, Crosshair, Cpu
} from 'lucide-react';
...
export default function WorkflowCenter() {
  ...
  if (routingTrace && routingTrace.output && routingTrace.output.selected_path) {
    ...
    // Support dynamic Sentinel and Meta
    ...
    // Filter STAGES but also we need to add definitions for sentinel/meta if they don't exist in STAGES
    const ALL_AVAILABLE_STAGES = [
      ...STAGES,
      { id: 'sentinel', label: 'Sentinel Security', icon: ShieldCheck, color: '#a855f7' },
      { id: 'meta', label: 'Meta-Agent Recovery', icon: Cpu, color: '#f59e0b' }
    ];
                  ...
                  {!activeTrace && activeStage !== 'approval' ? (
                    ...
                    >
                      ...
                      {activeStage === 'routing' && activeTrace?.output && (
                        <motion.div variants={itemVariants} className="glass-panel" style={{ background: 'linear-gradient(145deg, rgba(14,165,233,0.08), rgba(14,165,233,0.02))', border: '1px solid rgba(14,165,233,0.3)', borderRadius: '24px', padding: '40px', position: 'relative', overflow: 'hidden' }}>
                          <div style={{ position: 'absolute', top: '-100px', right: '-100px', width: '300px', height: '300px', background: 'radial-gradient(circle, rgba(14,165,233,0.1) 0%, transparent 70%)', filter: 'blur(40px)', zIndex: 0 }}></div>
                          
                          <div style={{ position: 'relative', zIndex: 1 }}>
                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '32px', borderBottom: '1px solid rgba(14,165,233,0.1)', paddingBottom: '24px' }}>
                              <div>
                                <h2 style={{ margin: '0 0 8px 0', color: '#38bdf8', fontSize: '28px', display: 'flex', alignItems: 'center', gap: '12px', fontWeight: 700 }}>
                                  <Network size={32} /> Adaptive Routing Engine
                                </h2>
                                <p style={{ margin: 0, color: '#94a3b8', fontSize: '15px' }}>Dynamic multi-agent pathway resolution</p>
                              </div>
                              <motion.div 
                                initial={{ scale: 0.8, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} transition={{ type: "spring" }}
                                style={{ background: 'linear-gradient(135deg, #0284c7, #0ea5e9)', color: 'white', padding: '12px 24px', borderRadius: '30px', fontSize: '15px', fontWeight: 800, letterSpacing: '1.5px', boxShadow: '0 8px 20px rgba(14,165,233,0.4)', border: '1px solid rgba(255,255,255,0.2)', textTransform: 'uppercase' }}
                              >
                            ...
                            <div style={{ display: 'grid', gridTemplateColumns: '1.5fr 1fr', gap: '32px', marginBottom: '32px' }}>
                              ...
                              <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
                                <motion.div variants={itemVariants} style={{ background: 'rgba(0,0,0,0.3)', padding: '28px', borderRadius: '16px', borderLeft: '4px solid #0ea5e9' }}>
                                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
                                    <h4 style={{ fontSize: '12px', color: '#94a3b8', textTransform: 'uppercase', margin: 0, letterSpacing: '1px' }}>Routing Selection Reason</h4>
                                    <div style={{ background: 'rgba(16,185,129,0.15)', color: '#34d399', padding: '4px 12px', borderRadius: '20px', fontSize: '12px', fontWeight: 700, border: '1px solid rgba(16,185,129,0.3)' }}>
                                      CONFIDENCE: {activeTrace.output.confidence || '99%'}
                                    </div>
                                  </div>
                                ...
                                <motion.div variants={itemVariants}>
                                  <h4 style={{ fontSize: '12px', color: '#94a3b8', textTransform: 'uppercase', marginBottom: '16px', letterSpacing: '1px' }}>Patient Factors Evaluated</h4>
                                  <ul style={{ margin: 0, padding: 0, listStyle: 'none', display: 'flex', flexDirection: 'column', gap: '10px' }}>
                                    {activeTrace.output.factors_evaluated?.map((factor: string, i: number) => (
                                      <motion.li 
                                        key={i} 
                                        initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.1 }}
                                        style={{ display: 'flex', gap: '12px', fontSize: '15px', color: '#bae6fd', alignItems: 'center', background: 'rgba(14,165,233,0.05)', padding: '12px 16px', borderRadius: '10px', border: '1px solid rgba(14,165,233,0.1)' }}
                                      >
                              ...
                              {/* Right Column: Scorecard & Map */}
                              <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
                                <motion.div variants={itemVariants} style={{ background: 'rgba(0,0,0,0.2)', padding: '24px', borderRadius: '16px', border: '1px solid rgba(255,255,255,0.05)' }}>
                                  <h4 style={{ fontSize: '12px', color: '#94a3b8', textTransform: 'uppercase', margin: '0 0 20px 0', letterSpacing: '1px' }}>Routing Scorecard</h4>
                                  <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                                    {activeTrace.output.scorecard && Object.entries(activeTrace.output.scorecard).map(([label, score]: [string, any], i: number) => (
                                      <div key={i}>
                                        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '8px', fontSize: '13px', color: '#cbd5e1' }}>
                                          <span>{label}</span>
                                          <span style={{ fontWeight: 600, color: score > 75 ? '#ef4444' : score > 40 ? '#f59e0b' : '#10b981' }}>{score}/100</span>
                                        </div>
                                        <div style={{ height: '6px', background: 'rgba(255,255,255,0.1)', borderRadius: '3px', overflow: 'hidden' }}>
                                          <motion.div 
                                            initial={{ width: 0 }} animate={{ width: `${score}%` }} transition={{ duration: 1, delay: i * 0.1 }}
                                            style={{ height: '100%', background: score > 75 ? '#ef4444' : score > 40 ? '#f59e0b' : '#10b981', borderRadius: '3px' }}
                                          />
                                        </div>
                                      </div>
                                    ))}
                                ...
                                <motion.div variants={itemVariants} style={{ background: 'rgba(14,165,233,0.1)', padding: '24px', borderRadius: '16px', border: '1px solid rgba(14,165,233,0.3)', flex: 1, display: 'flex', flexDirection: 'column' }}>
                                  <h4 style={{ fontSize: '12px', color: '#94a3b8', textTransform: 'uppercase', margin: '0 0 16px 0', letterSpacing: '1px' }}>Visual Route Map</h4>
                                  <div style={{ color: '#f1f5f9', fontSize: '14px', lineHeight: '2', fontWeight: 600, fontFamily: 'monospace', background: 'rgba(0,0,0,0.3)', padding: '20px', borderRadius: '12px', flex: 1, display: 'flex', flexWrap: 'wrap', alignItems: 'center', justifyContent: 'center', textAlign: 'center' }}>
                                    {(activeTrace.output.selected_path || 'Triage → Routing → Pharma → Scheduler → Bed').split('→').map((node: string, i: number, arr: any[]) => (
                                      <span key={i} style={{ display: 'inline-flex', alignItems: 'center' }}>
                                        <span style={{ color: node.trim() === 'Routing' ? '#38bdf8' : '#e2e8f0' }}>{node.trim()}</span>
                                        {i < arr.length - 1 && <span style={{ color: '#64748b', margin: '0 8px' }}>→</span>}
                                      </span>
                                    ))}
                                  </div>
                                </motion.div>
